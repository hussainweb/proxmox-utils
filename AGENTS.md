...
- **Proxmox Provider Authentication:** The Terraform configurations use the `telmate/proxmox` provider. Assume that authentication variables (`PM_API_URL`, `PM_USER`, `PM_PASS`) are injected via the user's environment. Do not hardcode credentials anywhere in the code.
