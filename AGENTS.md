...
- **Proxmox Provider Authentication:** The Terraform configurations use the `bpg/proxmox` provider. Assume that authentication variables (`PROXMOX_VE_ENDPOINT`, `PROXMOX_VE_USERNAME`, `PROXMOX_VE_PASSWORD`) are injected via the user's environment. Do not hardcode credentials anywhere in the code.
